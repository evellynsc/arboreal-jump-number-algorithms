...
std::list<my_graph::vertex> minimal_extension::get_min_z(
...
std::map<my_graph::vertex, std::list<my_graph::vertex>>
minimal_extension::find_x2_z(my_graph::vertex c, my_graph::vertex x1) {
...
minimal_extension::find_edges_to_remove(
    std::map<my_graph::vertex, std::list<my_graph::vertex>>& map_x2_minz) {
    auto removed_edges =
        std::list<std::pair<my_graph::vertex, my_graph::vertex>>();
    auto map_x2_z =
    ...
    std::cout << "\n(x2 = "
              << problem_instance.covering_graph[selected_jump.first].id + 1
              << "\t z = "
              << problem_instance.covering_graph[selected_jump.second].id + 1
              << ")" << std::endl;
    return map_x2_z[selected_jump];
}

void minimal_extension::update_extension(
...
void minimal_extension::update_violations(
...
std::list<my_graph::vertex> minimal_extension::difference_x1_x2(
...
my_graph::digraph minimal_extension::get_solution() { return this->solution; }
