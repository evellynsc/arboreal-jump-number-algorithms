...
BranchAndCut::BranchAndCut(ajns::Instance& _instance)
    : Optimizer(_instance, EXPONENTIAL, false) {}

BranchAndCut::BranchAndCut(ajns::Instance& _instance, bool _relaxed, bool _with_cutset_constraints, SolverParameters& _parameters)
    : Optimizer(_instance, EXPONENTIAL, _relaxed, _parameters) {
        this->with_cutset_constraints = _with_cutset_constraints;
    }

void BranchAndCut::add_variables() {
    this->x = new GRBVar[this->instance.num_edges];
...
void BranchAndCut::add_constraints() {
...
void BranchAndCut::add_limit_indegree_constraints() {
...
void BranchAndCut::add_number_of_edges_constraints() {
...
void BranchAndCut::add_out_edges_constraints() {
...
void BranchAndCut::add_cutset_constraints() {
...
void BranchAndCut::fix_arcs_value() {
    auto count = 0u;
    for (const auto& e :
         boost::make_iterator_range(boost::edges(instance.input_graph))) {
...
bool BranchAndCut::in_the_set(unsigned _set, std::size_t _element) {
    return (_set & (1 << _element));
}

GRBVar* BranchAndCut::get_variables_x() { return this->x; }

GRBVar* BranchAndCut::get_variables(int which) { return this->x; }

void BranchAndCut::extract_solution() {}

void BranchAndCut::add_callbacks() {
...
void BranchAndCut::setup() {
    Optimizer::setup();
    this->gurobi_model->set(GRB_IntParam_LazyConstraints, 1);
    this->gurobi_model->set(GRB_IntParam_PreCrush, 1);
