...
Instance instance_generator::create_instance(problem_data &data) {
    ...
    auto order_graph =
        create_order_graph(vertex_properties, data.adjacency_matrix);
    auto covering_graph = create_covering_graph(order_graph);

    // Remove vértices que formam um caminho o-o-o-o, onde o último vértice é
    // uma folha
    ...
    auto input_graph =
        create_input_graph(order_graph, covering_graph, artificial_arcs_pair);
    ...
    auto ajnp =
        Instance(data.id, root, order_graph, t_order_graph, covering_graph,
                 input_graph, predecessors, sucessors, covering_predecessors,
                 covering_sucessors, artificial_arcs_pair);
    ...
    boost::write_graphviz(outFile, order_graph,
                          ...
                          boost::make_label_writer(boost::get(
                              &my_graph::edge_info::type, order_graph)));
    outFile.close();
    ...
    boost::write_graphviz(outFile, covering_graph,
                          ...
                          boost::make_label_writer(boost::get(
                              &my_graph::edge_info::type, covering_graph)));
    outFile.close();
    ...
    boost::write_graphviz(outFile, input_graph,
                          ...
                          boost::make_label_writer(boost::get(
                              &my_graph::edge_info::type, input_graph)));
    outFile.close();
    return ajnp;
}
...
void instance_generator::fix_ids_order_graph(
    my_graph::digraph &order_graph, my_graph::digraph &covering_graph) {
    for (const auto &e :
         boost::make_iterator_range(boost::edges(order_graph))) {
        if (order_graph[e].id == -1) order_graph[e].id = current_edge_id++;
    }
}
...
void instance_generator::add_root(
    ...
    for (auto i = 0u; i < n; i++) {
        for (auto j = 0u; j < n; j++) {
            if (adjacency_matrix[i][j] == true) minimum[j] = false;
        }
    }
...
/* CREATE ORDER GRAPH */
my_graph::digraph instance_generator::create_order_graph(
    std::vector<my_graph::vertex_info> &v_info,
    std::vector<std::vector<bool>> &adjacency_matrix) {
    auto n = v_info.size();
        ...
        for (auto j = 0u; j < n; j++) {
            if (adjacency_matrix[i][j]) {
                boost::tie(e, inserted) = boost::add_edge(
                    ...
                    my_graph::edge_info(i, j, my_graph::ORIGINAL),
                    adj_matrix_graph);
                if (inserted) {
                    inserted = false;
                } else {
    ...
    for (auto &e : g_to_tc) order_graph[e.second] = adj_matrix_graph[e.first];
...
// TODO: review
my_graph::digraph instance_generator::create_covering_graph(
    ...
    boost::transitive_reduction(
        order_graph, aux_graph, boost::make_assoc_property_map(g_to_tr),
        boost::get(&my_graph::vertex_info::index, order_graph));
    ...
    for (const auto &e :
         boost::make_iterator_range((boost::edges(aux_graph)))) {
...
instance_generator::find_incompatible_vertices(my_graph::digraph &order_graph) {
    auto incompatible_vertices =
        std::vector<std::pair<my_graph::vertex, my_graph::vertex>>();
    ...
    for (auto h : boost::make_iterator_range(boost::vertices(order_graph))) {
        for (auto t :
            ...
            if (inserted.empty() or inserted.find({t, h}) == inserted.end()) {
                auto not_root =
                    not(order_graph[h].is_root or order_graph[t].is_root);
...
my_graph::digraph instance_generator::create_input_graph(
    my_graph::digraph &order_graph, my_graph::digraph &covering_graph,
    std::vector<std::pair<int, int>> &art_arcs_pair) {
    ...
    for (auto a : incompatible_vertices) {
        ...
        boost::tie(e, inserted) = boost::add_edge(
            a.first, a.second,
            my_graph::edge_info(current_edge_id++, a.first, a.second,
                                my_graph::ARTIFICIAL, value_set, 0),
            input_graph);

        if (not inserted) exit(0);
        boost::tie(e, inserted) = boost::add_edge(
            a.second, a.first,
            my_graph::edge_info(current_edge_id++, a.second, a.first,
                                my_graph::ARTIFICIAL, value_set, 0),
            input_graph);
        value_set = false;
    ...
    assert(boost::num_edges(input_graph) ==
           boost::num_edges(covering_graph) + 2 * incompatible_vertices.size());
...
std::vector<bool> instance_generator::find_ids_maximum_vertices(
    ...
    for (auto i = 0u; i < n; i++) {
        ...
        for (auto j = 0u; j < n; j++) {
            ...
            if (is_maximum) maximum_vertices[i] = true;
        }
    }
...
void instance_generator::update_maximum_vertices(my_graph::digraph &graph,
                                                 std::vector<bool> &ids) {
    for (auto v : boost::make_iterator_range(boost::vertices(graph))) {
        if (ids[graph[v].id]) graph[v].is_maximum = true;
    }
}

my_graph::digraph instance_generator::transpose_order_graph(
    my_graph::digraph &order_graph) {
