...
class instance_generator {
    ...
    void fix_ids_order_graph(my_graph::digraph &order_graph,
                             my_graph::digraph &covering_graph);
    std::vector<my_graph::vertex_info> create_vertex_properties(
        std::vector<std::string> &vertex_labels);
    void add_root(std::vector<std::vector<bool>> &adjacency_matrix,
                  std::vector<std::string> &vertex_labels,
                  std::size_t &root_id);
    my_graph::digraph create_order_graph(
        std::vector<my_graph::vertex_info> &vertex_info,
    ...
    find_incompatible_vertices(my_graph::digraph &order_graph);
    void add_artificial_edges();
    my_graph::digraph create_input_graph(my_graph::digraph &order_graph,
                                         my_graph::digraph &covering_graph,
                                         std::vector<std::pair<int, int>> &);
    my_graph::digraph transpose_order_graph(my_graph::digraph &order_graph);
    std::vector<bool> find_ids_maximum_vertices(
        std::vector<std::vector<bool>> &adjacency_matrix);
    void update_maximum_vertices(my_graph::digraph &graph,
                                 std::vector<bool> &ids);
