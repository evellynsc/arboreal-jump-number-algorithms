...
//CPX_ON); 	cplex_solver.setParam(IloCplex::Param::MIP::Strategy::RINSHeur,
//CPX_ON); 	cplex_solver.setParam(IloCplex::Param::MIP::Strategy::FPHeur,
//CPX_ON);
////	cplex.setParam(IloCplex::Param::Preprocessing::Linear, 0);
//	cplex_solver.setParam(IloCplex::Param::MIP::Strategy::Search,
//CPX_MIPSEARCH_TRADITIONAL); 	cplex_solver.setParam(IloCplex::Param::Threads,
//4); 	cplex_solver.setParam(IloCplex::Param::TimeLimit, config.time_limit);
...
void BCSolver::solve() {
    try {
        ...
        if (this->config.add_initial_solution) {
            ...
            for (boost::tie(eit, eit_end) = boost::edges(h_solution);
                 eit != eit_end; ++eit) {
        ...
        if (cplex_solver.solve()) {
            ...
            std::cout << "# Solution status: " << cplex_solver.getStatus()
                      << std::endl;
            std::cout << "# Arboreal jump number = "
                      << cplex_solver.getObjValue() << std::endl;
            std::cout << "================================================\n";
            std::cout << "================================================\n";
            std::cout << "================================================\n";
            ...
            edges); for (auto e :
            boost::make_iterator_range(boost::edges(final_solution))) { auto
            head = final_solution[e].source_id; auto tail =
            final_solution[e].target_id; std::cout << head << ',' << tail << ' '
            << final_solution[e].type << std::endl;
            }
            ...
            //problem_instance.id + "_output.ps"; 			std::system(command.c_str());
        } else {
            std::cout << "============================\n";
