...
ILOUSERCUTCALLBACK2(add_min_cuts_uc, IloBoolVarArray, x, ajns::Instance&,
                    problem_instance) {
    ...
    /*std::ofstream
    outFile1("C:/Users/evellynsc/Desktop/user_cut_relx_capacity.dot",
    std::ios_base::app);
    ...
    std::ofstream outFile2("C:/Users/evellynsc/Desktop/user_cut_relx_type.dot",
    std::ios_base::app);
    ...
    for (const auto& e : boost::make_iterator_range(
        ...
        if (not boost::edge(head, tail, tc_graph_x).second and
            ...
            for (const auto& v :
                ...
                // remove v (actually, we delete arcs going out or coming into
                // v)
