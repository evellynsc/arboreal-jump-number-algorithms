...
ILOUSERCUTCALLBACK2(separate_precedence_inequalities, IloBoolVarArray, x,
    ...
    for (auto v : boost::make_iterator_range(
        ...
        if (not problem_instance.input_graph[v].is_maximum) {
            ...
            if (graph_pi_flow.get_max_flow_value() < 1) {
                ...
                auto pred_set_s = get_adjacent_vertices_bool(
                    set_s, problem_instance.t_order_graph);
        ...
        if (not problem_instance.input_graph[v].is_root) {
            ...
            if (graph_sigma_flow.get_max_flow_value() < 1) {
                ...
                auto succ_bar_s = get_adjacent_vertices_bool(
                    set_bar_s, problem_instance.order_graph);
    ...
    for (auto e : boost::make_iterator_range(
        ...
        if (not graph_x[head].is_root) {
            ...
            //				callback::strengthen_cut(min_cut, succ_bar_s,
            //graph_sigma);
