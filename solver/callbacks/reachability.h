...
ILOLAZYCONSTRAINTCALLBACK2(find_constraints_for_integral_solution,
                           IloBoolVarArray, x, ajns::Instance&,
                           problem_instance) {
    ...
    auto cuts =
        std::map<my_graph::edge, std::pair<std::vector<my_graph::vertex>,
                                           std::vector<my_graph::vertex>>>();
    //	std::cout << "=======================\n";
    //	boost::print_graph(problem_instance.order_graph);

    auto cuts_to_add = std::vector<
        std::pair<std::set<my_graph::vertex>, std::set<my_graph::vertex>>>();
    for (const auto& e : boost::make_iterator_range(
        ...
        if (not boost::edge(head, tail, tc_graph_x).second) {
            ...
            auto set_complement = callback::get_set_complement(
                boost::vertices(problem_instance.input_graph), set_s);
            ...
            auto tail_predecessors = callback::get_neighboors_of(tail,
            problem_instance.input_graph); tail_predecessors.insert(tail);
            set_complement =
            callback::get_set_complement(boost::vertices(problem_instance.input_graph),
                                tail_predecessors);*/
    ...
    for (auto c : cuts_to_add) {
        ...
        for (auto h : heads) {
            for (auto t : tails) {
                boost::tie(e, exist) =
                    boost::edge(h, t, problem_instance.input_graph);
                if (exist) {
    ...
    ////		heads.push_back(boost::source(c.first,
    ...
    //					edges_of_cut.push_back(boost::edge(h,t,
    //problem_instance.input_graph).first);
    //				}
    //			}
    //		}
    //		auto lhside_ineq = IloExpr(env);
