...
IloNumVarArray DDLModel::get_v_variables() { return y; }

IloNumVarArray DDLModel::get_x_variables() { return x; }
...
void DDLModel::add_variables() {
    ...
    for (auto i = 0; i < n; i++) {
        for (auto j = 0; j < n; j++) {
            auto edge_in_input_graph =
                problem_instance.exist_edge_in_input_graph(i, j);
...
void DDLModel::add_constraints() {
    ...
    for (auto i = 0u; i < n; i++) {
        for (auto j = 0u; j < n; j++) {
            std::cout << x[j + n * i].getName()
                      << " == " << y[j + n * i].getName() << std::endl;
    ...
    for (auto i = 0u; i < n; i++) {
        for (auto j = 0u; j < n; j++) {
            ...
            for (auto t = 0u; t < t_max + 1; t++) {
                std::cout << y[j + n * i + t * n * n].getName() << " + "
                          << y[i + n * j + t * n * n].getName() << " <= 1"
                          << std::endl;
                cplex_model.add(
    ...
    for (auto i = 0u; i < n; i++) {
        for (auto j = 0u; j < n; j++) {
            for (auto t = 1u; t < t_max + 1; t++) {
                ...
                if (i != j) {
                    std::cout
                        << y[j + n * i + t * n * n].getName()
                        << " <= " << y[j + n * i + (t - 1) * n * n].getName()
                        << sk << std::endl;
                    cplex_model.add(y[j + n * i + t * n * n] <=
                                    y[j + n * i + (t - 1) * n * n] + sum_k);
                }
            }
        }
    }
    ...
    for (auto i = 0u; i < n; i++) {
        for (auto j = 0u; j < n; j++) {
            if (i != j) {
                for (auto t = 1u; t < t_max + 1; t++) {
                    // std::cout << y[j + n * i + t * n * n].getName() << " >= "
                    // << y[j + n * i + (t - 1) * n * n].getName() << std::endl;
                    cplex_model.add(y[j + n * i + t * n * n] >=
                                    y[j + n * i + (t - 1) * n * n]);
                }
            }
    ...
    for (auto i = 0u; i < n; i++) {
        for (auto k = 0u; k < n; k++) {
            for (auto j = 0u; j < n; j++) {
                ...
                for (auto t = 1u; t < t_max + 1; t++) {
                    /*	cout << i << " " << k << " " << j << " " << t << " " <<
                       std::endl; std::cout << y[j + n * i + t * n *
                       n].getName() << " >= " << u[k + n * i + n * n * j + (t -
                       1) * n * n * n].getName() << std::endl;*/
                    cplex_model.add(
                        y[j + n * i + t * n * n] >=
                        u[k + n * i + n * n * j + (t - 1) * n * n * n]);
                }
    ...
    for (auto i = 0u; i < n; i++) {
        for (auto k = 0u; k < n; k++) {
            for (auto j = 0u; j < n; j++) {
                ...
                for (auto t = 0u; t < t_max + 1; t++) {
                    // cout << i << " " << k << " " << j << " " << t << " " <<
                    // std::endl; std::cout << u[i + n * k + n * n * j + t * n *
                    // n * n].getName() << " <= " << y[i + n * k + t * n *
                    // n].getName() << std::endl; std::cout << u[i + n * k + n *
                    // n * j + t * n * n * n].getName() << " <= " << y[j + n * i
                    // + t * n * n].getName() << std::endl; std::cout << u[i + n
                    // * k + n * n * j + t * n * n * n].getName() << " >= " <<
                    //	y[i + n * k + t * n * n].getName() << " + " << y[j + n *
                    // i + t * n * n].getName() << " - 1" << std::endl;

                    cplex_model.add(u[i + n * k + n * n * j + t * n * n * n] <=
                                    y[i + n * k + t * n * n]);
                    cplex_model.add(u[i + n * k + n * n * j + t * n * n * n] <=
                                    y[j + n * i + t * n * n]);
                    cplex_model.add(u[i + n * k + n * n * j + t * n * n * n] >=
    ...
    // for (auto i = 0u; i < n; i++) {
    // 	for (auto j = 0u; j < n; j++) {
    // 		auto edge_in_order = problem_instance.exist_edge_in_order_graph(i,
    // j); 		if (edge_in_order and i != j) { 			std::cout << y[j + n
    // * i + t_max * n
    // * n].getName() << " == 1" << std::endl; 			cplex_model.add(y[j + n
    // * i + t_max * n * n] == 1);
    // 		}
    // 	}
    // }
