...
CharacterizationBasedFormulation::CharacterizationBasedFormulation(
    ajns::Instance& problem_instance, int s) {
    this->type = CHARACTERIZATION;
...
CharacterizationBasedFormulation::CharacterizationBasedFormulation(
    ajns::Instance& problem_instance, int s, bool _linear_relaxation) {
    this->type = CHARACTERIZATION;
...
IloNumVarArray CharacterizationBasedFormulation::get_x_variables() { return x; }

IloArray<IloNumVarArray> CharacterizationBasedFormulation::get_a_variables() {
    return a;
}

IloNumVarArray CharacterizationBasedFormulation::get_r_variables() { return r; }

int CharacterizationBasedFormulation::get_s() { return s; }

IloNumVarArray CharacterizationBasedFormulation::get_f_variables() { return f; }

IloNumVarArray CharacterizationBasedFormulation::get_g_variables() { return g; }
        ...
        if (j != problem_instance.root) {
            ...
            for (int t = 0; t <= s; t++) {
                if (problem_instance.covering_predecessors.at(j).size() > 0) {
                    for (auto i :
                         problem_instance.covering_predecessors.at(j)) {
                        auto idx_m = index_parser_m[i][j];
                        sum_a += a[idx_m][t];
                    }
                }
