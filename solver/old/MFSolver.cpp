...
void MFSolver::solve() {
    try {
        ...
        if (cplex_solver.solve()) {
            ...
            std::cout << "================================================\n";
            std::cout << "# Solution status: " << cplex_solver.getStatus()
                      << std::endl;
            ...
            for (auto e :
            boost::make_iterator_range(boost::edges(problem_instance.covering_graph)))
            { auto i = problem_instance.covering_graph[e].source_id; auto j =
            problem_instance.covering_graph[e].target_id; std::cout << i+1 << ",
            " << j+1 << std::endl;
            }
            ...
            for (auto const& e : boost::make_iterator_range(
                ...
                if (cplex_solver.getValue(model.y[n * i + j]) > 1e-6) {
                    ...
                    std::cout
                        << i << "->" << j
                        << " [label=" << problem_instance.input_graph[e].type
                        << ",value="
                        << cplex_solver.getValue(model.y[n * i + j]) << "];\n";
                    // std::cout << n * i + j << ',' << i + 1 << ", " << j + 1
                    // << " " << cplex_solver.getValue(model.y[n * i + j]) << "
                    // " << problem_instance.input_graph[e].type << std::endl;
                }
            }
            ...
            for (auto const& e : boost::make_iterator_range(
                     boost::edges(problem_instance.order_graph))) {
                auto i = problem_instance.input_graph[e].source_id;
                auto j = problem_instance.input_graph[e].target_id;
            ...
            for (auto k = 0; k < n - 1; k++) {
                for (auto e :
            boost::make_iterator_range(boost::edges(problem_instance.input_graph)))
            { auto i = problem_instance.input_graph[e].source_id; auto j =
            problem_instance.input_graph[e].target_id;

                    if (cplex_solver.getValue(model.x[i + j*n + n*n*k]) > 1e-6)
            { std::cout << i + 1 << ", " << j + 1 << ", " << k + 1 << " " <<
            cplex_solver.getValue(model.x[i + j * n + n * n * k]) << std::endl;
                    }
                }
            }*/
            ...
            auto final_solution =
            ajns::solution::construct_from_edges(problem_instance.input_graph,
            edges); std::ofstream outFile; auto name_file = problem_instance.id
            + "_output.dot"; outFile.open(name_file);
            boost::write_graphviz(outFile, final_solution,
                boost::make_label_writer(boost::get(&my_graph::vertex_info::id,
            final_solution)),
                boost::make_label_writer(boost::get(&my_graph::edge_info::type,
            final_solution))); outFile.close();*/
            //			auto command = "dot -Tps " + name_file + " -o " +
            // problem_instance.id + "_output.ps";
