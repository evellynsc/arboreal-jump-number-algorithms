...
MultiFlowModel::MultiFlowModel(ajns::Instance& problem_instance_,
                               bool linear_relaxation_) {
    this->type = solver::COMPACT;
    this->problem_instance = problem_instance_;
    this->linear_relaxation = linear_relaxation_;
}

IloNumVarArray MultiFlowModel::get_y_variables() { return y; }
...
void MultiFlowModel::add_constraints() {
    ...
    for (auto v : boost::make_iterator_range(
        ...
        if (not problem_instance.input_graph[v].is_root) {
            ...
            for (boost::tie(in_begin, in_end) =
                     boost::in_edges(v, problem_instance.input_graph);
    ...
    for (auto v : boost::make_iterator_range(
        ...
        if (not problem_instance.input_graph[v].is_root) {
            ...
            for (boost::tie(in_begin, in_end) =
                     boost::out_edges(v, problem_instance.input_graph);
                 in_begin != in_end; ++in_begin) {
        ...
        for (auto k = 0u; k < n; k++) {
            if (k != r and k != j and j != r) {
                ...
                for (boost::tie(in_begin, in_end) =
                     ...
                     in_begin != in_end; ++in_begin) {
                    auto u =
                        boost::source(*in_begin, problem_instance.input_graph);
                    auto i = problem_instance.input_graph[u].id;
                ...
                for (boost::tie(out_begin, out_end) =
                    ...
                    auto u =
                        boost::target(*out_begin, problem_instance.input_graph);
                    auto i = problem_instance.input_graph[u].id;
